...
public class PersonRepository {
    ...
    */
    ...
    //2.13 Trả về danh sách 'developer' ở các thành phố Tokyo, Stockholm
    public Map<String, List<Person>> developerListInCity() {
        return people.stream()
                .filter(p -> p.getJob().equals("developer") && (p.getCity().equals("Tokyo") || p.getCity().equals("Stockholm")))
                .collect(Collectors.groupingBy(Person::getCity));
    }

    //2.14 Tìm ra thành phố có độ tuổi trung bình của lập trình viên trẻ nhất
    public Map.Entry<String, Double> youngestdevs() {
        Map<String, Double> map = people
                .stream()
                .filter(p -> p.getJob().equals("developer"))
                .collect(Collectors.groupingBy(Person::getCity, Collectors.averagingInt(Person::getAge)));

        Map.Entry<String, Double> min = Collections.min(map.entrySet(), Comparator.comparing(Map.Entry::getValue));

        return min;
    }

    //2.15 Liệt kê tỷ lệ nam/nữ ở từng thành phố
    public Map<String, Map<String, Long>>  malefemaleratioEachCity() {
    ...
    //2.16 Tính mức lương trung bình của tất cả những người trên 30 tuổi
